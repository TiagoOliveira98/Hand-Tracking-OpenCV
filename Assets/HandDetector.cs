...
public class HandDetector : MonoBehaviour
{
    ...
    Mat HandMat;
    [SerializeField]public BoxOutline Outline;//changed here to public
    [SerializeField]public BoxOutline hitBox;
    public float Xmin;
    public float Xmax;
    public float Ymax;
    public float Ymin;
    ...
    public int test;
    public int hitXmax, hitYmax, hitXmin, hitYmin;
    ...
    void Start()
    {
        ...
        point.x = UnityEngine.Random.Range(20, 1260);
        point.y = UnityEngine.Random.Range(20, 740);
        hover = false;
        xCenter = 0f;
        yCenter = 0f;
    ...
    void Update()
    {
        ...
        if (Outline != null)
        {
            xCenter = Outline.XMin + (Outline.XMax - Outline.XMin) / 2f;
            yCenter = Outline.YMin + (Outline.YMax - Outline.XMin) / 2f;
            test = (int)xCenter;
            //hitBox.XMax = (int)xCenter -/*+*/ 100;
            //hitBox.XMin = (int)xCenter +/*-*/ 100;
            //hitBox.YMax = (int)yCenter -/*+*/ 100;
            //hitBox.YMin = (int)yCenter +/*-*/ 100;
            //
            /*hitXmax = (int)hitBox.XMin;//was Max
            hitXmin = (int)hitBox.XMax;//was Min
            hitYmax = (int)hitBox.YMin;//was Max
            hitYmin = (int)hitBox.YMax;//was Min */
            /*hitXmax = (int)xCenter + 20;
            hitXmin = (int)xCenter - 20;
            hitYmax = (int)yCenter + 20;
            hitYmin = (int)yCenter - 20;*/
        }

        PointHover(Outline, point);
        //PointHover(hitBox, point);
        //
        if (/*PointHover(Outline, point)==false*/hover == false || findHand == false)
        {
            here = true;
            PostPoint(HandTexture, HandMat, point);
            //
            Utils.matToTexture2D(HandMat, HandTexture, false);
        }
        else
        {
            here = false;
        ...
        Utils.matToTexture2D(HandMat, HandTexture);
        if (Outline != null)
        {
            Xmax = Outline.XMax;
            Xmin = Outline.XMin;
        ...
        //Create a point to put the hand on
        /*x = point.x;
        y = point.y;*/
        /*Imgproc.circle(HandMat, point, 2, new Scalar(255, 0, 0), 1);
        Utils.matToTexture2D(HandMat, HandTexture);*/
    }

    //Function to verify if teh middle of the hand is in the Point
    /*bool*/ void PointHover(BoxOutline Outline, Point point)
    {
        ...
        //if(point.x > Outline.XMax/*xCenter-10*/ && point.x < Outline.XMin/*xCenter+10*/)
        /*{
            check1 = true;
        }*/
       // if(point.y > Outline.YMax/*yCenter - 10*/ && point.y < Outline.YMin/*yCenter + 10*/)
        /*{
            check2 = true;
        }
        ...
        else if(point.x > Outline.XMax/*xCenter-10*/ && point.x < Outline.XMin/*xCenter+10*/ && point.y > Outline.YMax/*yCenter - 10*/ && point.y < Outline.YMin/*yCenter + 10*/)
        {
            hover = true;
        }
        else
    ...
    //
    void PostPoint(Texture2D tex, Mat mat, Point point)
    {
        if (point != null)
        {
            Imgproc.circle(mat, point, 1, new Scalar(255, 0, 0), 10);
        }
        Utils.matToTexture2D(mat, tex, false, 0,false);
    }
    ...
    void PostAction(Texture2D tex, Mat mat, BoxOutline outl)
    {
        ...
        Utils.matToTexture2D(mat, tex, false);
    }
